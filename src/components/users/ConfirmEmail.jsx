  ...
  useEffect(() => {
    ...
    userApiURI.userConfirmation(data).then((response) => {
      if(response.status === 200 && response.data.success === true){
        setMsgRetorno(<Result status="success" title="Cadastro confirmado com sucesso!" subTitle="A plataforma te espera! Seja bem vindo(a)!" extra={<Link to='/login'><Button type="primary">Login</Button></Link>} />)
        setLoading(false)
